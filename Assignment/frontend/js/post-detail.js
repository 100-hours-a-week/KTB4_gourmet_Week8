...
try {
    const response = await fetch(`${API_BASE_URL}/posts/${postId}/likes/users/${userId}`);

    if (!response.ok) {
        console.log("좋아요 상태 조회 실패");
        updateLikeUi();
        return;
    }
...
try {
    ...
    if (!response.ok) {
        ...
        console.log("좋아요 처리 실패:", errorData);
...
commentItem.innerHTML = `
    <div class="comment-top">
        <div class="comment-writer">
            ...
            <strong>${getCommentWriter(comment)}</strong>
            <span class="comment-date">${getCommentCreatedAt(comment)}</span>
        </div>
    ...
    <p class="comment-content">${getCommentContent(comment)}</p>
`;
...
const commentWriterImage = commentItem.querySelector(".writer-image");

if (commentWriterImage && comment.profileImage) {
...
try {
    ...
    if (!response.ok) {
        ...
        console.log("게시글 삭제 실패:", errorData);
...
if (editingCommentId) {
    try {
        const response = await fetch(`${API_BASE_URL}/posts/${postId}/comments/${editingCommentId}`, {
            method: "PATCH",
            headers: {
        ...
        if (!response.ok) {
            ...
            console.log("댓글 수정 실패:", errorData);
            alert("댓글 수정에 실패했습니다.");
            return;
        }
...
try {
    ...
    if (!response.ok) {
        ...
        console.log("댓글 등록 실패:", errorData);
        alert("댓글 등록에 실패했습니다.");
        return;
    }
...
try {
    const response = await fetch(`${API_BASE_URL}/posts/${postId}/comments/${deletingCommentId}`, {
        method: "DELETE"
    });
    ...
    if (!response.ok) {
        ...
        console.log("댓글 삭제 실패:", errorData);
        alert("댓글 삭제에 실패했습니다.");
        return;
    }
